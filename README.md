...
Historical Python client for SAS Customer Intelligence 360 **data-management** APIs, retained as a reference. It is part of the `sas-ci360` family of solution packages, alongside packages for planning, identity, execution, content delivery, and workflow.
...
`sasci360soldata` wraps the CI 360 Marketing Data REST API with a thin `Data` class built on top of the shared `sasci360apicore` (connection, encryption, logging) and `sasci360apimarketingdata` packages. It's meant to be dropped into automation scripts or scheduled jobs that need to manage CI 360 marketing data objects programmatically rather than through the UI.
...
Key dependencies: `pandas`, `PyJWT`, `requests`, `saspy`, `schedule`, `DateTime`, `urllib3`, plus the sibling `sasci360apicore` and `sasci360apimarketingdata` packages.
...
```python
from sasci360soldata import Data
